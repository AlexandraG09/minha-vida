(function () {
    ...
    function iniciarTarefas() {
        elementos = {
            botoesSecao: document.querySelectorAll(
                "[data-secao-casa]"
            ),

            paineisSecao: document.querySelectorAll(
                "[data-painel-casa]"
            ),
            ...
            campoDivisao: document.querySelector(
                "#tarefa-divisao"
            ),

            campoPrioridade: document.querySelector(
                "#tarefa-prioridade"
            ),
            ...
            totalConcluidas: document.querySelector(
                "#tarefas-concluidas"
            ),

            totalInicio: document.querySelector(
                "#total-tarefas-inicio"
            )
        };
    ...
    function configurarSeparadores() {
        elementos.botoesSecao.forEach(
            function (botao) {
                botao.addEventListener(
                    "click",
                    function () {
                        const secao =
                            botao.dataset.secaoCasa;
                        ...
                        elementos.paineisSecao.forEach(
                            function (painel) {
                                painel.classList.toggle(
                                    "ativo",
                                    painel.dataset.painelCasa ===
                                        secao
                                );
                            }
                        );
    ...
    async function abrirEdicao(id) {
        try {
            ...
            elementos.campoDivisao.value =
                tarefa.divisao;

            elementos.campoPrioridade.value =
                tarefa.prioridade;
    ...
    async function guardarTarefa(evento) {
        ...
        const tarefa = {
            ...
            dataLimite:
                elementos.campoData.value || null,

            notas:
                elementos.campoNotas.value.trim(),

            concluida:
                tarefaExistente?.concluida || false,
    ...
    function ordenarTarefas(
        ...
        if (
            ...
            return primeira.dataLimite.localeCompare(
                segunda.dataLimite
            );
        }
    ...
    function criarLinhaTarefa(tarefa) {
        ...
        artigo.className = "linha-tarefa";

        if (tarefa.concluida) {
            artigo.classList.add("concluida");
        } else if (estaAtrasada(tarefa)) {
            artigo.classList.add("atrasada");
        }
        ...
        botaoEstado.className = "estado-tarefa";
        ...
        detalhe.textContent =
            `${tarefa.divisao} · ${tarefa.prioridade}`;
        ...
        botaoApagar.dataset.id = tarefa.id;
    ...
    function atualizarResumo(tarefas) {
        ...
        elementos.totalConcluidas.textContent =
            String(concluidas);
    ...
    window.MinhaVidaTarefas = Object.freeze({
        atualizar: atualizarTarefas
    });
