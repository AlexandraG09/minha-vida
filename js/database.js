(function () {
    ...
    function abrirBaseDados() {
        ...
        promessaBaseDados = new Promise(
            function (resolver, rejeitar) {
                if (!("indexedDB" in window)) {
                    rejeitar(
                        new Error(
                            "Este navegador não permite armazenamento local."
                        )
                    );
                ...
                pedido.onblocked = function () {
                    ...
                    rejeitar(
                        new Error(
                            "A base de dados está bloqueada por outra janela."
                        )
                    );
                };
            }
    ...
    function criarLojaSeNecessario(
        baseDados,
        nomeLoja,
        chave
    ) {
        if (
            !baseDados.objectStoreNames.contains(
                nomeLoja
            )
        ) {
            baseDados.createObjectStore(
                nomeLoja,
                {
                    keyPath: chave
                }
    ...
    async function executarOperacao(
        ...
        return new Promise(
            function (resolver, rejeitar) {
                const transacao = baseDados.transaction(
                    nomeLoja,
                    modo
                );
    ...
    window.MinhaVidaDB = Object.freeze({
        LOJAS: LOJAS,
        abrirBaseDados: abrirBaseDados,
        guardar: guardar,
        obter: obter,
        listar: listar,
        remover: remover,
        limparLoja: limparLoja,
